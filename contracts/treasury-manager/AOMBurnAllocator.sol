...
contract AOMBurnAllocator is Ownable {
    ...
    mapping( address => tokenData ) public tokenInfo; // info for reserve token to burn AOM

    uint public totalBought; // total AOM bought
    uint public totalBurnt; // total AOM burnt

    uint public immutable timelockInBlocks; // timelock to raise deployment limit

    bool public enableSendback;

    address public immutable AOM=0x5C4FDfc5233f935f20D2aDbA572F770c2E377Ab0;
    ...
    address public immutable daiAOM=0xbc0eecdA2d8141e3a26D2535C57cadcb1095bca9;
    address public immutable usdcAOM=0xd661952749f05aCc40503404938A91aF9aC1473b;
    ...
    function burnLp( address token, uint amount ) public onlyPolicy() {
        ...
        (,uint stableReserve)=AOMStableAmount(IPair(lpToken));
        ...
        (uint stableAmount,uint AOMAmount)=
        ...
        IERC20(AOM).burn(bought.add(AOMAmount));
        
        // account for burn
        accountingFor( lpToken, lpAmount, stableAmount, bought, bought.add(AOMAmount) );
    }
    ...
    function addToken( address token, uint max ,uint transactionLimit) external onlyPolicy() {
        require(token==dai||token==usdc||token==daiAOM||token==usdcAOM,"principle token invalid");
     ...
     *  @param AOMBought uint
     */
    function accountingFor( address token, uint tokenSpent, uint stableSpent, uint AOMBought, uint AOMBurnt ) internal {
        ...
        tokenInfo[ token ].AOMBought = tokenInfo[ token ].AOMBought.add(AOMBought);
        tokenInfo[ token ].AOMBurnt = tokenInfo[ token ].AOMBurnt.add(AOMBurnt);
        totalBurnt = totalBurnt.add( AOMBurnt );
        totalBought = totalBought.add( AOMBought );
    }
    ...
    function priceMeetCriteria() public view returns (bool){
        //backingPerAOM decimals = 4, 101.23$ = 1012300, 75.8321$ = 758321
