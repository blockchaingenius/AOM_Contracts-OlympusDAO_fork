...
contract AOMBondStakeDepository is Ownable {
    ...
    address public immutable AOM; // intermediate reward token from treasury
    address public immutable sAOM; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sAOM gons remaining to be paid
        uint AOMPayout; //AOM amount at the moment of bond
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISAOM(sAOM).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sAOM ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sAOM balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISAOM(sAOM).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISAOM(sAOM).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
